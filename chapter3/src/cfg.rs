...
struct Production(Symbol, Vec<Vec<Symbol>>);
impl fmt::Display for Production {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for item in self.1.iter() {
            let symbols: Vec<String> = item.iter().map(|s| s.to_string()).collect();
    ...
    while is_change {
        ...
        for production in productions.iter() {
            for item in &production.1 {
                ...
                for i in 0..item.len() {
                    let yi = unsafe { item.get_unchecked(i) };
                    ...
                    for j in (i + 1)..item.len() {
                        let yj = unsafe { item.get_unchecked(j) };

                        // if Yi+1...Yj-1 都是可空的 then FOLLOW[Yi] = FOLLOW[Yi] U FOLLOW[Yj]
                        if j_is_nullable {
                            let first_set_yj =
                                first_set.get(yj).cloned().unwrap_or_else(HashSet::new);
                    ...
                    if j_is_nullable {
                        ...
                        is_change =
                            follow_yi_set_origin_len != follow_set_yi_set.len() || is_change;
                    }
                }
